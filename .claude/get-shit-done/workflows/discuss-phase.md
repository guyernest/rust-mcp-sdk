...
```bash
INIT=$(node "/Users/guy/Development/mcp/sdk/rust-mcp-sdk/.claude/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
```bash
TODO_MATCHES=$(node "/Users/guy/Development/mcp/sdk/rust-mcp-sdk/.claude/get-shit-done/bin/gsd-tools.cjs" todo match-phase "${PHASE_NUMBER}")
```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "/Users/guy/Development/mcp/sdk/rust-mcp-sdk/.claude/get-shit-done/bin/gsd-tools.cjs" resolve-model gsd-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @/Users/guy/Development/mcp/sdk/rust-mcp-sdk/.claude/agents/gsd-advisor-researcher.md for your role and instructions.
...
```bash
node "/Users/guy/Development/mcp/sdk/rust-mcp-sdk/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "/Users/guy/Development/mcp/sdk/rust-mcp-sdk/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
```bash
node "/Users/guy/Development/mcp/sdk/rust-mcp-sdk/.claude/get-shit-done/bin/gsd-tools.cjs" config-set workflow._auto_chain_active true
```
