...
- [~] **Phase 113: Stateless HTTP + Multi-Round-Trip Elicitation** — v2 requests run handshake-free/session-free on the existing `stateless()` branch; MRTR (`input_required`/`requestState`/`inputResponses`) end-to-end; opt-in `subscriptions/listen` (server + client halves); no SSE resumability + id-replay regression test; the pmcp `Client` speaks v2 and fulfills MRTR. **All 20 plans shipped and every phase gate is green** — the original 13, plus the four-plan gap-closure round (113-17 SSE parser bound / 113-18 listen refusal + semaphore prune / 113-20 collected-body cap / 113-19 fuzz-seam gating + phase gate) that answered `113-VERIFICATION.md`'s GAP-A..E and the `113-REVIEW.md` findings. **The progress table's "Complete" for this phase means all 20 plans shipped; the phase itself is BLOCKED ON PUBLICATION, not complete:** the final `schema/2026-07-28` had still not published as of 2026-07-27, so the three v2 error-code constants remain pre-final values held under a written developer exception and HTTP-01..05 / CLNT-01..02 stay `[~]`. Re-run the `113-SPEC-RECHECK.md` checkpoint on or after 2026-07-28 to close it.
