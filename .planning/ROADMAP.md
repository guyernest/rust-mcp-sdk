...
- [ ] 117-06-PLAN.md — Wave 2. Prove the repo's FIRST `#[cfg_attr(…, path = …)]` paired module on a minimal payload, whole-file gate `src/shared/event_store.rs`, add the SEMANTIC null-twin check (no state held, no state/header operation, nothing declared the real module lacks) (SMPL-01/SMPL-02; D-03/A-D03)
- [ ] 117-07-PLAN.md — Wave 2. Two-attempt era-pinned `client_for` classified by a TYPED reachability outcome built before stringification, additive `EffectTrace` version field plumbed end-to-end, deterministic `ReplayInvoker` era-mismatch guard, `pmcp-agent` 0.2.0 with both workspace pins updated (CLNT-03; D-07/D-08/A-D08/D-09)
...
| 117. Agents, Tester & v1 Severability | 4/14 | In Progress|  |
