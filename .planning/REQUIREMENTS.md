...
- [x] **WBVER-01**: Text & boolean formula output cells render as formula-with-cached-result (`Formula::set_result`), so Excel (`fullCalcOnLoad="1"`) independently recomputes ALL output types on open — not just numeric formulas
- [x] **WBVER-02**: `render_workbook` accepts `mode: filled | inputs_only` (default `filled`, additive); `inputs_only` emits bare formulas with NO cached results, deterministically; unknown mode → `Err` (never a panic); the encoded `workbook://` URI stays ≤ 64 KiB
- [x] **WBVER-03**: A `verify_accuracy` meta-tool re-runs the executor at the workbook's reference inputs and returns a per-output reconciliation report (`{key, server_value, oracle_value, abs_delta, within_tol}`) vs. `Tool.oracle` within `TOL` — stateless, reader-free, the compile-time penny-reconcile made runtime-inspectable
- [x] **WBVER-04**: No regression to existing `render_workbook` / `workbook://` behavior or wire shapes; ALWAYS coverage (fuzz/property/unit/example); `make quality-gate` + `make purity-check` + `make doc-check` green; PMAT cog-25
...
## v2.10 Requirements — Phase 102: lift the task lifecycle onto the high-level `Server` / HTTP path

**Goal:** Phase 101 put the `tasks/*` lifecycle on `ServerCore` only; the HTTP-facing `Server::handle_request` hard-rejects `tasks/*` (`src/server/mod.rs:1166-1169`), so HTTP-hosted servers (pmcp.run's Lambdas) can only serve tasks by driving `ServerCore::handle_request` via a shim. Lift the lifecycle onto the high-level `Server`/`StreamableHttpServer` by SHARING one implementation between `Server` and `ServerCore` (not duplicating it). Additive; the `tasks/*` wire contract is frozen by Phase 101.

- [x] **HTASK-01**: The high-level `ServerBuilder`/`Server` accepts a `TaskStore` and honors `with_task_support` tools with Phase 101's endpoint-backed capability rule (auto-advertise top-level `tasks` only when `task_store.is_some() || task_router.is_some()`; a `TaskSupport::Required` tool with no backend → `build()` Err; explicit capabilities never clobbered) — reusing the existing `apply_tasks_capability_rule` implementation, not a re-derivation
- [x] **HTASK-02**: `Server::handle_request` serves the full task lifecycle — the task-augmented `tools/call` create-path (store-minted id on wire `task.taskId` + `_meta`, terminal-result persistence) and `tasks/get | result | list | cancel` (store-first typed serialization, `TaskRouter` fall-through, `-32002` pending error, owner-scoping via `resolve_task_owner`) — replacing the hard-reject at `mod.rs:1166-1169`. The lifecycle logic is a SINGLE shared unit used by both `Server` and `ServerCore`, NOT a divergent second copy
- [x] **HTASK-03**: A server built with `Server::builder().tool(t, task_tool).task_store(store).build()` and served via `StreamableHttpServer` serves `initialize → call(task) → tasks/get (poll) → tasks/result` over REAL HTTP, verified by a live HTTP loopback round-trip (real `StreamableHttpServer` + `StreamableHttpTransport` on `127.0.0.1:0`) asserting non-empty typed content, three-way id consistency (`CreateTaskResult.task.taskId == tasks/get id == _meta.relatedTask.taskId`), advertised `tasks` capability, and the pending-result error — replacing Phase 101's in-process duplex harness
- [x] **HTASK-04**: No regression to the `ServerCore` task path (all Phase 101 tests green) or existing `Server`/HTTP behavior; no breaking change to public wire shapes (`Task`/`GetTaskResult`/`CallToolResult`/`CreateTaskResult`/`ServerCapabilities`) or the `Server`/`ServerBuilder` API (additive); `#[cfg(not(target_arch="wasm32"))]` preserved; worked HTTP example (`s46_http_tool_as_task`) + ALWAYS coverage; `make quality-gate` and `make doc-check` green; a pmcp.run-shaped HTTP server serves tasks with NO `ServerCore::handle_request` shim
...
| HTASK-01 | Phase 102 | Complete |
| HTASK-02 | Phase 102 | Complete |
| HTASK-03 | Phase 102 | Complete |
| HTASK-04 | Phase 102 | Complete |
...
| WBVER-01 | Phase 100 | Complete |
| WBVER-02 | Phase 100 | Complete |
| WBVER-03 | Phase 100 | Complete |
| WBVER-04 | Phase 100 | Complete |
