...
must_haves:
  truths:
    ...
    - "TTL from client task params is respected and passed to the store"
    - "progressToken from original request is available in the background task execution"
  artifacts:
   ...
   ClientRequest::CallTool(req) => {
       ...
       if let Some(ref task_router) = self.task_router {
           let needs_task = req.task.is_some()
               || task_router.tool_requires_task(&req.name, req.execution_meta());
           if needs_task {
