...
|---------|------|------|-------------|----------|-----------|-------------------|-------------|--------|
| TBD | TBD | 1 | SMPL-01 | `full` and `full-v2` differ by exactly `v1-compat`, **derived** from `Cargo.toml` | unit | `cargo test --test v1_severability_tripwire` | ❌ W0 | ⬜ pending |
| TBD | TBD | 1 | SMPL-01 | `v1-compat` is present in BOTH `default` and `full` | unit | `cargo test --test v1_severability_tripwire` | ❌ W0 | ⬜ pending |
| TBD | TBD | 1 | SMPL-01 / SMPL-02 | Crate compiles with the **real transport** and NO v1 layer | build | `RUSTFLAGS="-D warnings" cargo build -p pmcp --no-default-features --features full-v2` | ❌ W0 (new `ci.yml` job) | ⬜ pending |
| TBD | TBD | 1 | SMPL-01 | The severance job is reachable from `gate.needs` and actually blocks | **manual-only** | Scratch PR that breaks `full-v2`; assert `gate` reports failure | ❌ manual by necessity | ⬜ pending |
| TBD | TBD | 2 | SMPL-02 | The `full-v2` build contains no session/SSE-resumability code | unit (source tripwire) | `cargo test --test v1_severability_tripwire` — assert the v1 module file carries no `sessions`/`event_store`/`Last-Event-ID` token, **with a non-vacuity guard on file length** | ❌ W0 | ⬜ pending |
| TBD | TBD | 2 | SMPL-02 | The v1 wire is **byte-identical** across the cut | integration (golden) | `cargo test --test v1_byte_identity_after_cut --features "full"` | ❌ W0 — **capture goldens BEFORE the cut** | ⬜ pending |
| TBD | TBD | 2 | SMPL-02 | `sessions_active` / `resumability_active` truth tables survive the move | unit + property | `cargo test --lib --features "full" sessions_active` / `resumability_active` | ✅ `streamable_http_server.rs:4568,4585,5879,5894` | ⬜ pending |
| TBD | TBD | 2 | SMPL-02 | v2 exchanges write zero event-store traffic and replay nothing | integration (spy) | `cargo test --lib --features "full" spy_records` | ✅ `streamable_http_server.rs:5946,5972,5997,6015` | ⬜ pending |
| TBD | TBD | 2 | SMPL-02 | A v2 response is never routed into a session SSE stream | integration | `cargo test --lib --features "full" v2_response_is_never_routed` | ✅ `streamable_http_server.rs:6060` | ⬜ pending |
| TBD | TBD | 1 | SMPL-01 | The sunset-policy rustdoc compiles warning-free | doc | `make doc-check` (after adding `v1-compat` to `Makefile:429`) | ✅ target exists; ❌ feature-list edit | ⬜ pending |
| TBD | TBD | 3 | CLNT-03 | Agent connects to a v2 server end-to-end (tools/list → tools/call → task poll → terminal) | integration (live socket) | `cargo test --test agent_v2_e2e --features "full"` | ❌ W0 | ⬜ pending |
| TBD | TBD | 3 | CLNT-03 | Agent **falls back to v1** when the server answers-and-rejects v2 | integration (live socket) | `cargo test --test agent_v2_e2e --features "full" fallback` | ❌ W0 — **the D-07 negative case; must not be skipped** | ⬜ pending |
| TBD | TBD | 3 | CLNT-03 | An unreachable host **propagates** rather than reporting era V1 | integration | `cargo test --test agent_v2_e2e --features "full" unreachable` | ❌ W0 | ⬜ pending |
| TBD | TBD | 3 | CLNT-03 | A pre-117 `EffectTrace` (no era field) still deserializes; a `None` era serializes byte-identically | unit | `cargo test -p pmcp-agent trace` | ✅ harness `trace.rs:221-227`; new cases needed | ⬜ pending |
| TBD | TBD | 3 | CLNT-03 | `ReplayInvoker` fails deterministically on an era mismatch | property | `cargo test -p pmcp-agent --test replay_safety` | ✅ `tests/replay_safety.rs` (AGNT-03); new case needed | ⬜ pending |
| TBD | TBD | 3 | CLNT-03 | `pmcp-agent` still builds for wasm32 under default features | build | `cargo build -p pmcp-agent --target wasm32-unknown-unknown` | ✅ `ci.yml:374-377` (in `gate.needs`) | ⬜ pending |
| TBD | TBD | 3 | CLNT-04 | Dual-run detects a both-era server and emits a comparison | integration (live socket) | `cargo test -p mcp-tester --test dual_run` | ❌ W0 | ⬜ pending |
| TBD | TBD | 3 | CLNT-04 | Single-run stdout is **BYTE-IDENTICAL** to 0.7.0 for both `--format pretty` and `--format json` | integration (golden) | `cargo test -p mcp-tester --test report_compat` | ❌ W0 — the D-11 additivity proof | ⬜ pending |
| TBD | TBD | 3 | CLNT-04 | `cargo-pmcp` still compiles against the changed `mcp-tester` | build | `cargo build -p cargo-pmcp` | ✅ implicit via `make build` — make it an explicit acceptance item | ⬜ pending |
| TBD | TBD | 3 | CLNT-04 | Every baseline entry has a unique id and a non-empty `source`; count ≥ 14 | unit | `cargo test -p mcp-tester --test era_baseline` | ❌ W0 | ⬜ pending |
| TBD | TBD | 1/3 | ALWAYS (CLAUDE.md) | Fuzz target for the baseline / feature-list parser | fuzz | `cargo fuzz run <target>` | ❌ W0 | ⬜ pending |
| TBD | TBD | 3 | ALWAYS (CLAUDE.md) | Runnable example: agent against a v2 server | example | `cargo run --example s49_v2_agent_client --features "full"` | ❌ W0 (follows `s47`/`s48` precedent) | ⬜ pending |
...
- [ ] `crates/mcp-tester/baselines/era-deltas.{toml,yaml}` — 14 seeded entries, each with `source`
...
- [ ] `examples/s49_v2_agent_client.rs` (or next free number) — CLAUDE.md ALWAYS runnable example
      (CLNT-03).
...
- [ ] No watch-mode flags
- [ ] Any nextest command uses `binary(...)`, never `test(/.../)`
- [ ] Severance build reported **separately** from `make quality-gate` every wave
