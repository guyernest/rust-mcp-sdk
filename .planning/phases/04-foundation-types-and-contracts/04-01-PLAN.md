...
Find all struct literal constructions of `GetPromptResult` (research identified ~3 locations around lines 832, 864, 973). Add `_meta: None` to each one. This is a non-behavioral change -- adding a None-valued optional field does not change any execution flow or message content.

**NOTE:** This is the ONLY modification to `prompt_handler.rs`. The zero-diff constraint from CONTEXT.md is interpreted as behavior-identical, not file-content-identical. Adding `_meta: None` to struct literals is mechanical and non-behavioral. The file's execution flow, message generation, and all observable behavior remain identical to v1.0.

**Also search the entire codebase** for any other `GetPromptResult` struct literal constructions (tests, examples, other handlers) and add `_meta: None` to those as well. Use `cargo check` to find any compilation errors from the missing field.
...
Run: `cargo test --workspace` -- full test suite passes.
  ...
  <done>
GetPromptResult has `_meta: Option<Map<String, Value>>` field. All existing struct literal sites updated with `_meta: None`. Serialization skips the field when None. Deserialization tolerates missing field. All existing tests pass unchanged (backward compatibility proven). prompt_handler.rs behavior is identical to v1.0.
  </done>
</task>
