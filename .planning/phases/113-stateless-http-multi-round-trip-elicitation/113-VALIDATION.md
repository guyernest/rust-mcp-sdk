...
35 tasks across 12 plans in 6 waves. Every task carries a real `<automated>` command — there
...
| 113-01-T2 | 01 | 1 | HTTP-02 | T-113-SC | Blocking human gate: `ring` + `zeroize` promotion adds zero crates; `zeroize_derive` absent | human-gate + lockfile assertion | `test "$(grep -c '^name = \"ring\"' Cargo.lock)" = "1" && test "$(grep -c '^name = \"zeroize\"' Cargo.lock)" = "1" && test "$(grep -c '^name = \"zeroize_derive\"' Cargo.lock)" = "0"` | yes (Cargo.lock) | ⬜ pending |
| 113-01-T3 | 01 | 1 | HTTP-01, HTTP-02 | T-113-SC, T-113-05, T-113-08, T-113-13 | `ring`+`zeroize` reachable as DIRECT deps (E0432 blocker closed); v2 codes centralized, never bare literals | unit + dep-graph assertion | `cargo build --lib --features streamable-http && cargo test --lib --features full -- error_codes && test "$(grep -c '^name = \"ring\"' Cargo.lock)" = "1" && test "$(grep -c '^name = \"zeroize\"' Cargo.lock)" = "1" && test "$(grep -c '^name = \"zeroize_derive\"' Cargo.lock)" = "0" && cargo tree -p pmcp -e normal --features streamable-http --depth 1 \| grep -q 'ring v0.17' && cargo tree -p pmcp -e normal --features streamable-http --depth 1 \| grep -q 'zeroize v1.8'` | yes (Cargo.toml, src/types/protocol/error_codes.rs) | ⬜ pending |
...
| 113-05-T1 | 05 | 2 | CLNT-01 | T-113-12, T-113-21 | Explicit v2 opt-in, no auto-probe; `_meta.clientInfo` is never identity | unit (tdd) | `cargo test --lib --features full -- client` | yes (src/client/mod.rs) | ⬜ pending |
| 113-05-T2 | 05 | 2 | CLNT-01 | T-113-06, T-113-08, T-113-20 | Three required v2 headers emitted; session id suppressed; pathological names cannot panic | unit (tdd) | `cargo test --lib --features full -- streamable_http && cargo build --lib --target wasm32-unknown-unknown` | yes (src/shared/streamable_http.rs) | ⬜ pending |
| 113-05-T3 | 05 | 2 | CLNT-01 | T-113-06, T-113-12 | Live client↔server proof that no `Mcp-Session-Id` crosses the wire on v2 | integration | `cargo test --test v2_client --features full` | created by task (tests/v2_client.rs) | ⬜ pending |
...
| 113-07-T1 | 07 | 3 | CLNT-02 | T-113-11 | Typed `MrtrRoundLimitExceeded` without an enum variant (no accidental semver break) | unit (tdd) + semver gate | `cargo test --lib --features full -- error:: && cargo semver-checks check-release --baseline-version 2.17.0 -p pmcp` | yes (src/error/mod.rs) | ⬜ pending |
| 113-07-T2 | 07 | 3 | CLNT-02 | T-113-26, T-113-25 | `inputRequests` dispatched only onto REGISTERED host handlers; no fabricated responses | unit (tdd) | `cargo test --lib --features full -- host` | yes (src/client/host/mod.rs) | ⬜ pending |
| 113-07-T3 | 07 | 3 | CLNT-02 | T-113-07, T-113-11, T-113-27, T-113-28 | Bounded gather→resend loop; fields land in `params`, not `_meta`; fresh id per round | unit (tdd) + integration | `cargo test --test v2_client --features full && cargo test --lib --features full -- client` | tests/v2_client.rs from 05-T3 | ⬜ pending |
...
| 113-11-T3 | 11 | 5 | HTTP-02, CLNT-02 | T-113-17, T-113-37 | Runnable example teaches the `PMCP_REQUEST_STATE_KEY` contract and never hardcodes a key | example run (timeout-124 contract) | `cargo build --example s47_v2_stateless_mrtr --features full && { timeout 12 cargo run --example s47_v2_stateless_mrtr --features full > target/s47_run.log 2>&1; test "$?" = "124"; } && grep -qE '127\.0\.0\.1:[0-9]+' target/s47_run.log` | created by task (examples/s47_v2_stateless_mrtr.rs) | ⬜ pending |
| 113-12-T1 | 12 | 6 | HTTP-01..05, CLNT-01..02 | T-113-38, T-113-39, T-113-40 | Crypto never reaches wasm; feature-unification false-greens caught; additive-only semver | build matrix + semver gate | `cargo build --lib --no-default-features && cargo build --lib --target wasm32-unknown-unknown && cargo semver-checks check-release --baseline-version 2.17.0 -p pmcp` | yes | ⬜ pending |
| 113-12-T2 | 12 | 6 | HTTP-01..05, CLNT-01..02 | T-113-41 | Toyota-Way gate: fmt, clippy pedantic+nursery, build, test, audit; complexity ≤25 | gate | `make quality-gate` | yes (Makefile) | ⬜ pending |
| 113-12-T3 | 12 | 6 | HTTP-01..05, CLNT-01..02 | T-113-42 | The `x-mcp-header` (SEP-2243) gap is recorded in BOTH ROADMAP and REQUIREMENTS, never silently absorbed | doc-artifact | `test "$(grep -c '^- \[x\] \*\*HTTP-0' .planning/REQUIREMENTS.md)" = "5" && grep -q 'x-mcp-header' .planning/REQUIREMENTS.md && grep -q 'x-mcp-header' .planning/ROADMAP.md` | yes (.planning/REQUIREMENTS.md, .planning/ROADMAP.md) | ⬜ pending |

*Status: ⬜ pending · ✅ green · ❌ red · ⚠️ flaky*

### Sampling continuity check

Longest run of consecutive tasks without an `<automated>` verify: **0**. Every one of the 35
...
- [x] No separate Wave-0 scaffold plan is needed. The one piece of shared harness that does
      ...
      every plan that consumes it (04, 05, 06, 08, 09, 10, 11 are all wave 2+).
- [x] No task references a test file that no task creates. Files created in-phase:
      ...
      `tests/v2_mrtr.rs`, `tests/v2_subscriptions.rs`,
...
- [x] All tasks have `<automated>` verify or Wave 0 dependencies — 35/35 have `<automated>`
...
**Approval:** approved (planner, 2026-07-24 — revision pass after checker feedback)
