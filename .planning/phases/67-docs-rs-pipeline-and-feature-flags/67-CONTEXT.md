...
- **D-13 (AMENDED 2026-04-11 post-research — row count corrected):** Individual feature rows (15 entries, alphabetized): `composition`, `http`, `http-client`, `jwt-auth`, `logging`, `macros`, `mcp-apps`, `oauth`, `rayon`, `resource-watcher`, `schema-generation`, `simd`, `sse`, `streamable-http`, `validation`, `websocket`. Total table rows = **17** (2 meta: `default` + `full`, plus 15 individual). `logging` gets its own row even though it's enabled by the `default` meta row — readers still need the "what does this feature do" description. Exclude `unstable`, `test-helpers`, `wasm`, `websocket-wasm`, `wasm-tokio`, `wasi-http`, and the three `*_example` gates — these are internal / build-time concerns not user-facing capabilities.
