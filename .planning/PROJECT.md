...
v2.10 SDK DX track — Phase 102 complete (2026-06-22): the `tasks/*` lifecycle is now available over the high-level `Server` and `StreamableHttpServer`, so an HTTP-hosted server (e.g. pmcp.run's Lambdas) serves task-based tools with NO `ServerCore::handle_request` shim. Phase 101 had put the whole lifecycle on `ServerCore` only and `Server::handle_request` hard-rejected `tasks/*`; Phase 102 extracts that machinery into ONE shared `src/server/task_dispatch.rs` unit (Option A — retiring the two-dispatcher drift) and wires it into `Server`/`ServerBuilder`: a store-backed `Server` auto-advertises `tasks` at `build()` (HTASK-01), the hard-reject is replaced by post-auth delegation to the shared `route_tasks_endpoint` (HTASK-02), and the create-path runs through the self-enforcing `maybe_build_task_created` gate. Proven by a LIVE HTTP loopback round-trip (ephemeral port + server-abort shutdown) plus HTTP-level cross-owner (IDOR) isolation and the worked `s46_http_tool_as_task` example (HTASK-03/04); verified 7/7 must-haves, all four req IDs traced, full lib suite 1093/1093 green. A code review flagged 2 follow-up polish items (bare `.unwrap()` at `task_dispatch.rs:409/446/480`; `resolve_owner` doc/behavior inversion at line 179 — does NOT affect tested paths) for a later cleanup. NOT yet PR'd/released.
...
---
*Last updated: 2026-06-22 — Phase 102 complete (HTTP task dispatch: shared `task_dispatch` unit wired into high-level `Server`/`StreamableHttpServer`; HTASK-01..04 validated, live HTTP round-trip + cross-owner isolation green, lib suite 1093/1093; 2 code-review polish follow-ups noted); previously 2026-06-15 — Phase 96 complete (Shape B `cargo pmcp new --kind workbook-server` scaffold + dialect-version declaration with both-lane fail-closed gate + WBEX-01 second-workbook served-schema generalization gate + WBEX-02 8-quirk corpus; WBCL-05/WBDL-02/WBEX-01/WBEX-02 validated, make quality-gate + purity-check green); previously 2026-06-14 — Phase 95 complete (pmcp-workbook-server Shape A pure-config binary mirroring pmcp-sql-server; test trio + --bundle-id proptest + reader-free purity gate + slot-9a wiring; requirement WBCL-06 validated)*
